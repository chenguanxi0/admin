...
//        echo chr(0xEF).chr(0xBB).chr(0xBF);   //只针对csv文件
    ...
    public function import(Request $request){
        ...
        foreach ($res as $k=>$v){
            if ($v[15]){
                // 1. 先判断该分类是否存在   --分类1
            $name = Category::where('name',$v[15])->get();
            if (!$name->first()){  //该name不存在,插入此条数据
                ...
                if($v[16]){ //判断上传的表当中是否有2级分类

                // 先判读二级分类是否存在数据库中，如果不存在就创建2 3 4级分类
                $name = Category::where('name',$v[16])->get();
                if (!$name->first()){
                    $category_2 = new Category;
                    $name_2 = Category::where('name',$v[15])->get()->first()->name;
                    $name_2 = Category::where('name',$name_2)->get();
                    ...
                    if($v[17]){
                        $name = Category::where('name',$v[17])->get();
                        if (!$name->first()){
                            $category_3 = new Category;
                            $name_3 = Category::where('name',$v[16])->get()->first()->name;
                            $name_3 = Category::where('name',$name_3)->get();
                            ...
                            if($v[18]){
                                $name = Category::where('name',$v[18])->get();
                                if (!$name->first()){
                                    $category_4 = new Category;
                                    $name_4 = Category::where('name',$v[17])->get()->first()->name;
                                    $name_4 = Category::where('name',$name_4)->get();
        ...
        foreach ($res as $k=>$v){
            ...
            $products[$k]['special_price'] = $v[6];
            $products[$k]['price'] = $v[9];
            $products[$k]['image'] = $v[2];

            //判断当前分类  默认存在一级分类
            if($v[16] == null){
                $products[$k]['category_id'] = Category::where('name',$v[15])->first()->id;
            }else{
                if($v[17] == null){
                    $products[$k]['category_id'] = Category::where('name',$v[16])->first()->id;
                }else{
                    if($v[18] == null){
                        $products[$k]['category_id'] = Category::where('name',$v[17])->first()->id;
                    }else{
                        $products[$k]['category_id'] = Category::where('name',$v[18])->first()->id;
                    }
                }
            }
        ...
        foreach ($res as $k=>$v){
                    ...
                    $product_descriptions[$k]['product_name'] = $v[3];
                    $product_descriptions[$k]['product_description'] = $v[4];
